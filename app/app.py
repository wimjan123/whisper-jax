...
def transcribe_youtube(yt_url, task=None, return_timestamps=False):
    html_embed_str = _return_yt_html_embed(yt_url)

    text, timestamps = inference(inputs=yt_url, task=task, return_timestamps=return_timestamps)
    output_download = gr.outputs.Download(label="Download as TXT", filename="output.txt", filecontent=text)
